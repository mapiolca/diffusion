...
/**
 * \file    diffusion/admin/setup.php
 * \ingroup diffusion
 * \brief   Diffusion setup page.
 */
...
/** @var HookManager $hookmanager */
$hookmanager->initHooks(array('diffusionsetup', 'globalsetup'));
...
// Setup conf for automatically setting diffusion status to sent after successful email sending
$item = $formSetup->newItem('DIFFUSION_AUTO_SET_SENT_ON_MAIL')->setAsYesNo();
$item->helpText = $langs->transnoentities('DIFFUSION_AUTO_SET_SENT_ON_MAILHelp');
...
$item = $formSetup->newItem('DIFFUSION_FULLACCESS_CONTACT')->setAsYesNo();
$item->helpText = $langs->transnoentities('DIFFUSION_FULLACCESS_CONTACTHelp');
...
if ($action == 'updateMask') {
	...
	foreach ($dirmodels as $reldir) {
		$file = dol_buildpath($reldir."core/modules/diffusion/doc/pdf_".$modele."_".strtolower($tmpobjectkey).".modules.php", 0);
		if (file_exists($file)) {
	...
	if ($className !== '') {
		...
		if ($module->write_file($tmpobject, $langs) > 0) {
			header("Location: ".DOL_URL_ROOT."/document.php?modulepart=diffusion&file=".strtolower($tmpobjectkey)."/SPECIMEN.pdf");
			return;
		} else {
	...
	if (!empty($tmpobjectkey)) {
		$constforval = 'DIFFUSION_'.strtoupper($tmpobjectkey)."_ADDON";
	...
	if ($ret > 0) {
		if (!empty($tmpobjectkey)) {
			$constforval = 'DIFFUSION_'.strtoupper($tmpobjectkey).'_ADDON_PDF';
...
$help_url = '';
$title = "DiffusionSetup";

llxHeader('', $langs->trans($title), $help_url, '', 0, 0, '', '', '', 'mod-diffusion page-admin');
...
print dol_get_fiche_head($head, 'settings', $langs->trans($title), -1, "diffusion@diffusion");

// Setup page goes here
echo '<span class="opacitymedium">'.$langs->trans("DiffusionSetupPage").'</span><br><br>';
					...
					while (($file = readdir($handle)) !== false) {
						if (strpos($file, 'mod_'.strtolower($myTmpObjectKey).'_') === 0 && substr($file, dol_strlen($file) - 3, 3) == 'php') {
							...
							if ($module->isEnabled()) {
								...
								$constforvar = 'DIFFUSION_'.strtoupper($myTmpObjectKey).'_ADDON';
							...
							if (preg_match('/\.modules\.php$/i', $file) && preg_match('/^(pdf_|doc_)/', $file)) {
								if (file_exists($dir.'/'.$file)) {
									...
									if ($modulequalified) {
										...
										// Default
										print '<td class="center">';
										$constforvar = 'DIFFUSION_'.strtoupper($myTmpObjectKey).'_ADDON_PDF';
