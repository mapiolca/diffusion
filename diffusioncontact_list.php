...
$permissiontoadd = (!empty($user->admin) || $user->hasRight('diffusion', 'diffusioncontact', 'write') || $user->hasRight('diffusion', 'diffusioncontact', 'write'));
$permissiontodelete = (!empty($user->admin) || $user->hasRight('diffusion', 'diffusioncontact', 'delete') || $user->hasRight('diffusion', 'diffusioncontact', 'delete'));
...
if ($num == 1 && getDolGlobalInt('MAIN_SEARCH_DIRECT_OPEN_IF_ONLY_ONE') && $search_all && !$page) {
	...
	$id = $obj->rowid;
	header("Location: ".dol_buildpath('/diffusion/diffusioncontact_card.php', 1).'?id='.((int) $id));
	exit;
}
...
$newcardbutton .= dolGetButtonTitle($langs->trans('New'), '', 'fa fa-plus-circle', dol_buildpath('/diffusion/diffusioncontact_card.php', 1).'?action=create&backtopage='.urlencode($_SERVER['PHP_SELF']), '', $permissiontoadd);
