...
dol_include_once('/diffusion/lib/diffusion_diffusioncontact.lib.php');
...
// Set $enablepermissioncheck to 1 to enable a minimum low level of checks
	...
	if ($enablepermissioncheck) {
		$permissiontoread = (!empty($user->admin) || $user->hasRight('diffusion', 'diffusioncontact', 'read') || $user->hasRight('diffusion', 'diffusioncontact', 'read'));
		$permissiontoadd = (!empty($user->admin) || $user->hasRight('diffusion', 'diffusioncontact', 'write') || $user->hasRight('diffusion', 'diffusioncontact', 'write')); // Used by the include of actions_addupdatedelete.inc.php and actions_lineupdown.inc.php
		$permissiontodelete = ((!empty($user->admin) || $user->hasRight('diffusion', 'diffusioncontact', 'delete') || $user->hasRight('diffusion', 'diffusioncontact', 'delete'))) || ($permissiontoadd && isset($object->status) && $object->status == $object::STATUS_DRAFT);
...
if (empty($reshook)) {
	$backurlforlist = dol_buildpath('/diffusion/diffusioncontact_list.php', 1);
	...
	if (empty($backtopage) || ($cancel && empty($id))) {
		if (empty($backtopage) || ($cancel && strpos($backtopage, '__ID__'))) {
			if (empty($id) && (($action != 'add' && $action != 'create') || $cancel)) {
				...
				$backtopage = dol_buildpath('/diffusion/diffusioncontact_card.php', 1).'?id='.((!empty($id) && $id > 0) ? $id : '__ID__');
			}
		}
	}

	$triggermodname = 'DIFFUSION_MYOBJECT_MODIFY'; // Name of trigger action code to execute when we modify record
	...
	// Action to build doc disabled for diffusion contacts
	...
	$triggersendname = 'DIFFUSION_MYOBJECT_SENTBYMAIL';
...
if ($object->id > 0 && (empty($action) || ($action != 'edit' && $action != 'create'))) {
	...
	// Object card
	// ------------------------------------------------------------
	$linkback = '<a href="'.dol_buildpath('/diffusion/diffusioncontact_list.php', 1).'?restore_lastsearch_values=1'.(!empty($socid) ? '&socid='.$socid : '').'">'.$langs->trans("BackToList").'</a>';
	...
	*/
	...
	if ($action != 'presend') {
		...
		if ($includedocgeneration) {
			...
			$filedir = $conf->diffusion->multidir_output[$conf->entity].'/diffusion/'.$object->element.'/'.$objref;
			$urlsource = $_SERVER["PHP_SELF"]."?id=".$object->id;
			$genallowed = $permissiontoread; // If you can read, you can build the PDF to read content
			$delallowed = $permissiontoadd; // If you can create/edit, you can remove a file on card
			print $formfile->showdocuments('diffusion:DiffusionContact', 'diffusion/'.$object->element.'/'.$objref, $filedir, $urlsource, $genallowed, $delallowed, $object->model_pdf, 1, 0, 0, 28, 0, '', '', '', $langs->defaultlang);
		}
		...
		$morehtmlcenter = dolGetButtonTitle($langs->trans('SeeAll'), '', 'fa fa-bars imgforviewmode', dol_buildpath('/diffusion/diffusioncontact_agenda.php', 1).'?id='.$object->id);
