...
if (empty($search_fk_user_exped) || !is_array($search_fk_user_exped)) {
...
if ($showentitycolumn) {
	$sqlentity = "SELECT rowid, label";
	$sqlentity .= " FROM ".MAIN_DB_PREFIX."entity";
	$sqlentity .= " WHERE rowid IN (".getEntity('diffusion', (GETPOSTINT('search_current_entity') ? 0 : 1)).")";
	$sqlentity .= " ORDER BY rowid";

	$resqlentity = $db->query($sqlentity);
	if ($resqlentity) {
		while ($objentity = $db->fetch_object($resqlentity)) {
			$entityfilteroptions[$objentity->rowid] = $objentity->label;
		}
		$db->free($resqlentity);
	}
}

$sqlsender = "SELECT rowid, lastname, firstname";
$sqlsender .= " FROM ".MAIN_DB_PREFIX."user";
$sqlsender .= " WHERE entity IN (".getEntity('user').")";
$sqlsender .= " ORDER BY lastname ASC, firstname ASC";

$resqlsender = $db->query($sqlsender);
if ($resqlsender) {
	while ($objsender = $db->fetch_object($resqlsender)) {
		$senderfilteroptions[$objsender->rowid] = dolGetFirstLastname($objsender->firstname, $objsender->lastname);
	}
	$db->free($resqlsender);
}
...
$permissiontoadd = (!empty($user->admin) || $user->hasRight('diffusion', 'diffusion', 'write') || $user->hasRight('diffusion', 'diffusion', 'write') || $user->hasRight('diffusion', 'write') || $user->hasRight('diffusion', 'write'));
$permissiontodelete = (!empty($user->admin) || $user->hasRight('diffusion', 'diffusion', 'delete') || $user->hasRight('diffusion', 'diffusion', 'delete') || $user->hasRight('diffusion', 'delete') || $user->hasRight('diffusion', 'delete'));
...
if (empty($reshook)) {
	...
	$uploaddir = $conf->diffusion->multidir_output[$conf->entity];
...
if ($num == 1 && getDolGlobalInt('MAIN_SEARCH_DIRECT_OPEN_IF_ONLY_ONE') && $search_all && !$page) {
	...
	$id = $obj->rowid;
	header("Location: ".dol_buildpath('/diffusion/diffusion_card.php', 1).'?id='.((int) $id));
	exit;
}
...
llxHeader('', $title, $help_url, '', 0, 0, $morejs, $morecss, '', 'mod-diffusion page-list bodyforlist');	// Can use also classforhorizontalscrolloftabs instead of bodyforlist for a horizontal scroll in the table instead of page
...
if ($showentitycolumn && !empty($search_entity)) {
	foreach ($search_entity as $searchentityid) {
		if ($searchentityid !== '') {
			$param .= '&search_entity[]='.urlencode($searchentityid);
		}
	}
}
...
foreach ($object->fields as $key => $val) {
	...
	if (!empty($arrayfields['t.'.$key]['checked'])) {
		...
		if ($key === 'fk_user_exped') {
			print $form->multiselectarray('search_'.$key, $senderfilteroptions, (isset($search[$key]) ? $search[$key] : array()), 0, 0, 'maxwidth100', 1);
