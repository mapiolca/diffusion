...
if ($action == 'presend') {
	...
	if ($object->element === 'diffusion') {
		...
		$sql .= ' WHERE sp.entity IN ('.getEntity('socpeople', $sharescopecontacts, $object).')';
		$sql .= " AND sp.email <> ''";
		$sql .= ' ORDER BY s.nom ASC, sp.lastname ASC, sp.firstname ASC';

		$resql = $db->query($sql);
		if ($resql) {
			while ($obj = $db->fetch_object($resql)) {
				$contactlabel = trim(dolGetFirstLastname($obj->firstname, $obj->lastname));
				if ($contactlabel === '') {
					$contactlabel = $obj->email;
				}
				if (!empty($obj->company)) {
					$contactlabel = $obj->company.' - '.$contactlabel;
				}
				$liste[(int) $obj->rowid] = $contactlabel.' <'.$obj->email.'>';
			}
		}
