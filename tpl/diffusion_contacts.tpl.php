...
// Build the list from llx_diffusion_contact and reuse native liste_contact only to resolve unlink rowid.
$list = array();
$linkrowidmap = array('internal' => array(), 'external' => array());
...
foreach (array('internal', 'external') as $source) {
	...
	foreach ((array) $contactlist as $contactline) {
		$linkkey = (int) $contactline['id'].'_'.(int) $contactline['fk_c_type_contact'];
		$linkrowidmap[$source][$linkkey] = (int) $contactline['rowid'];
		if (empty($linkrowidmap[$source][(int) $contactline['id']])) {
			$linkrowidmap[$source][(int) $contactline['id']] = (int) $contactline['rowid'];
		}
	}
}

$sql = 'SELECT dc.rowid as diffusioncontact_rowid, dc.fk_contact, dc.contact_source, dc.fk_type_contact, dc.mail_status, dc.letter_status, dc.hand_status, ctc.libelle as type_label';
$sql .= ' FROM '.MAIN_DB_PREFIX.'diffusion_contact as dc';
$sql .= ' LEFT JOIN '.MAIN_DB_PREFIX.'c_type_contact as ctc ON ctc.rowid = dc.fk_type_contact';
...
if ($resql) {
	while ($obj = $db->fetch_object($resql)) {
		$source = strtolower((string) $obj->contact_source);
		...
		$entry->status = 0;
		$entry->status_html = '';
		$entry->mail_status = (int) $obj->mail_status;
		$entry->letter_status = (int) $obj->letter_status;
		$entry->hand_status = (int) $obj->hand_status;
		$entry->source = $source;
		...
		if ($source == 'internal') {
			if ($userstatic->fetch($entry->contact_id) > 0) {
				$entry->contact_html = $userstatic->getNomUrl(-1, '', 0, 0, 0, 0, '', 'valignmiddle');
				$entry->contact_name = strtolower($userstatic->getFullName($langs));
				$entry->status = (int) $userstatic->statut;
				$entry->status_html = $userstatic->LibStatut($entry->status, 3);
			}
		} else {
			if ($contactstatic->fetch($entry->contact_id) > 0) {
				$entry->contact_html = $contactstatic->getNomUrl(1, '', 0, '', 0, 0);
				$entry->contact_name = strtolower($contactstatic->getFullName($langs));
				$entry->status = (int) $contactstatic->statut;
				$entry->status_html = $contactstatic->LibStatut($entry->status, 3);
				if ($contactstatic->socid > 0 && $companystatic->fetch($contactstatic->socid) > 0) {
...
foreach ($list as $entry) {
	...
	$diffusioncontactstatic->id = (int) $entry->diffusioncontact_rowid;
	$diffusioncontactstatic->element = 'diffusioncontact';
	$diffusioncontactstatic->table_element = 'diffusion_contact';
	$diffusioncontactstatic->diffusion = (int) $object->id;
	...
	if ($permission && $object->status == $object::STATUS_DRAFT && $entry->diffusioncontact_rowid > 0) {
