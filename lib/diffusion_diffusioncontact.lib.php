...
/**
 * \file    lib/diffusion_diffusioncontact.lib.php
 * \ingroup diffusion
...
function diffusioncontactPrepareHead($object)
{
	...
	$head[$h][0] = dol_buildpath("/diffusion/diffusioncontact_card.php", 1).'?id='.$object->id;
	$head[$h][1] = $langs->trans("DiffusionContact");
	$head[$h][2] = 'card';
	$h++;

	if ($showtabofpagecontact) {
		$head[$h][0] = dol_buildpath("/diffusion/diffusioncontact_contact.php", 1).'?id='.$object->id;
	...
	if ($showtabofpagenote) {
		if (isset($object->fields['note_public']) || isset($object->fields['note_private'])) {
			...
			$head[$h][0] = dol_buildpath('/diffusion/diffusioncontact_note.php', 1).'?id='.$object->id;
	...
	if ($showtabofpageagenda) {
		$head[$h][0] = dol_buildpath("/diffusion/diffusioncontact_agenda.php", 1).'?id='.$object->id;
	...
	//	'entity:-tabname:Title:@diffusion:/diffusion/mypage.php?id=__ID__'
	//); // to remove a tab
	complete_head_from_modules($conf, $langs, $object, $head, $h, 'diffusioncontact@diffusion');

	complete_head_from_modules($conf, $langs, $object, $head, $h, 'diffusioncontact@diffusion', 'remove');
