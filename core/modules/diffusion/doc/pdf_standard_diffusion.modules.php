...
/**
 *  \file       core/modules/diffusion/doc/pdf_standard.modules.php
 *  \ingroup    diffusion
...
class pdf_standard_diffusion extends ModelePDFDiffusion
{
	...
	public function write_file($object, $outputlangs, $srctemplatepath = '', $hidedetails = 0, $hidedesc = 0, $hideref = 0)
	{
		...
		$this->setEmitterFromObjectEntity($object, $outputlangs);
	       ...
	       if ($paths !== null) {
			$object->fetch_thirdparty();
		       ...
		       if (file_exists($dir)) {
				...
				$thirdpartyname = (is_object($object->thirdparty) && !empty($object->thirdparty->name)) ? $object->thirdparty->name : '';
				$pdf->SetKeyWords($outputlangs->convToOutputCharset($object->ref)." ".$outputlangs->transnoentities("Diffusion")." ".$outputlangs->convToOutputCharset($thirdpartyname));
	...
	/**
	 * Set emitter company according to object entity constants.
	 *
	 * @param Diffusion $object Diffusion object
	 * @param Translate $outputlangs Output language
	 * @return void
	 */
	protected function setEmitterFromObjectEntity($object, $outputlangs)
	{
		$entity = !empty($object->entity) ? (int) $object->entity : 1;
		$emetteur = clone $this->emetteur;
		$constbyname = $this->getCompanyConstantsByEntity($entity);

		$const2field = array(
			'MAIN_INFO_SOCIETE_NOM' => 'name',
			'MAIN_INFO_SOCIETE_NAME' => 'name',
			'MAIN_INFO_SOCIETE_ADDRESS' => 'address',
			'MAIN_INFO_SOCIETE_ZIP' => 'zip',
			'MAIN_INFO_SOCIETE_TOWN' => 'town',
			'MAIN_INFO_SOCIETE_STATE' => 'state',
			'MAIN_INFO_SOCIETE_PHONE' => 'phone',
			'MAIN_INFO_SOCIETE_FAX' => 'fax',
			'MAIN_INFO_SOCIETE_EMAIL' => 'email',
			'MAIN_INFO_SOCIETE_WEB' => 'url',
			'MAIN_INFO_SOCIETE_LOGO' => 'logo',
			'MAIN_INFO_SIREN' => 'idprof1',
			'MAIN_INFO_SIRET' => 'idprof2',
			'MAIN_INFO_NAF' => 'idprof3',
			'MAIN_INFO_RCS' => 'idprof4',
			'MAIN_INFO_TVAINTRA' => 'tva_intra',
		);

		foreach ($const2field as $constname => $fieldname) {
			if (isset($constbyname[$constname]) && $constbyname[$constname] !== '') {
				$emetteur->{$fieldname} = $constbyname[$constname];
			}
		}

		if (!empty($constbyname['MAIN_INFO_SOCIETE_COUNTRY'])) {
			$countryid = (int) $constbyname['MAIN_INFO_SOCIETE_COUNTRY'];
			if ($countryid > 0) {
				$sql = "SELECT rowid, code";
				$sql .= " FROM ".MAIN_DB_PREFIX."c_country";
				$sql .= " WHERE rowid = ".((int) $countryid);
				$resql = $this->db->query($sql);
				if ($resql) {
					$obj = $this->db->fetch_object($resql);
					if ($obj) {
						$emetteur->country_id = (int) $obj->rowid;
						$emetteur->country_code = (string) $obj->code;
					}
					$this->db->free($resql);
				}
			}
		...
		if (empty($emetteur->country_code)) {
			$emetteur->country_code = substr($outputlangs->defaultlang, -2);
		}
	...
	/**
	 * Get company constants resolved for an entity.
	 *
	 * @param int $entity Entity id
	 * @return array<string,string>
	 */
	protected function getCompanyConstantsByEntity($entity)
	{
		$constants = array();
		$sql = "SELECT name, value, entity";
		$sql .= " FROM ".MAIN_DB_PREFIX."const";
		$sql .= " WHERE name LIKE 'MAIN_INFO_SOCIETE_%'";
		$sql .= " AND entity IN (0, ".((int) $entity).")";
		$sql .= " ORDER BY entity ASC";
		$resql = $this->db->query($sql);
		if ($resql) {
			while ($obj = $this->db->fetch_object($resql)) {
				$constants[(string) $obj->name] = (string) $obj->value;
			}
			$this->db->free($resql);
		}
	...
	protected function normalizeDescriptionHtmlForPdf($descriptionHtml, $availableWidth)
	{
		...
		$layoutStyle = '<style>
table{width:auto !important;max-width:'.$maxWidth.'mm !important;table-layout:auto;border-collapse:collapse;}
...
img{max-width: '.$maxWidth.'mm !important;max-height:'.$maxHeight.'mm !important;width:auto !important;height:auto !important;}
	...
	*/
	...
	protected function _pagehead(&$pdf, $object, $showaddress, $outputlangs, $outputlangsbis = null)
	{
		...
		// Show Draft Watermark
		if (getDolGlobalString('DIFFUSION_DRAFT_WATERMARK') && $object->status == $object::STATUS_DRAFT) {
			pdf_watermark($pdf, $outputlangs, $this->page_hauteur, $this->page_largeur, 'mm', dol_escape_htmltag(getDolGlobalString('DIFFUSION_DRAFT_WATERMARK')));
		}
