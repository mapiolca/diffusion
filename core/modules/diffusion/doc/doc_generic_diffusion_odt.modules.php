...
/**
 *	\file       htdocs/core/modules/diffusion/doc/doc_generic_diffusion_odt.modules.php
 *	\ingroup    diffusion
...
class doc_generic_diffusion_odt extends ModelePDFDiffusion
{
	...
	public function __construct($db)
	{
		...
		$this->scandir = 'DIFFUSION_MYOBJECT_ADDON_PDF_ODT_PATH'; // Name of constant that is used to save list of directories to scan
	...
	public function info($langs)
	{
		...
		$texte .= '<input type="hidden" name="param1" value="DIFFUSION_MYOBJECT_ADDON_PDF_ODT_PATH">';
		$texte .= '<table class="nobordernopadding centpercent">';

		// List of directories area
		$texte .= '<tr><td>';
		$texttitle = $langs->trans("ListOfDirectories");
		$listofdir = explode(',', preg_replace('/[\r\n]+/', ',', trim(getDolGlobalString('DIFFUSION_MYOBJECT_ADDON_PDF_ODT_PATH'))));
		$listoffiles = array();
		...
		if (!getDolGlobalString('MAIN_NO_MULTIDIR_FOR_ODT')) {
			...
			$texte .= getDolGlobalString('DIFFUSION_MYOBJECT_ADDON_PDF_ODT_PATH');
			$texte .= '</textarea>';
			...
			$texte .= '<br>';
			$texte .= '<input type="hidden" name="value1" value="DIFFUSION_MYOBJECT_ADDON_PDF_ODT_PATH">';
		}

		// Scan directories
		$nbofiles = count($listoffiles);
		if (getDolGlobalString('DIFFUSION_MYOBJECT_ADDON_PDF_ODT_PATH')) {
		...
		if ($nbofiles) {
			...
			foreach ($listoffiles as $file) {
				$texte .= '- '.$file['name'].' <a href="'.DOL_URL_ROOT.'/document.php?modulepart=doctemplates&file=diffusion_diffusion/'.urlencode(basename($file['name'])).'">'.img_picto('', 'listlight').'</a>';
				$texte .= ' &nbsp; <a class="reposition" href="'.$_SERVER["PHP_SELF"].'?modulepart=doctemplates&keyforuploaddir=DIFFUSION_MYOBJECT_ADDON_PDF_ODT_PATH&action=deletefile&token='.newToken().'&file='.urlencode(basename($file['name'])).'">'.img_picto('', 'delete').'</a>';
				$texte .= '<br>';
			}
			$texte .= '</div>';
		}
		...
		$texte .= '<input type="hidden" value="DIFFUSION_MYOBJECT_ADDON_PDF_ODT_PATH" name="keyforuploaddir">';
	...
	public function write_file($object, $outputlangs, $srctemplatepath = '', $hidedetails = 0, $hidedesc = 0, $hideref = 0)
	{
		...
		if (!empty($conf->diffusion->multidir_output[$entityfordoc])) {
			...
			if (file_exists($dir)) {
				...
				dol_mkdir($conf->diffusion->dir_temp);
				if (!is_writable($conf->diffusion->dir_temp)) {
					$this->error = $langs->transnoentities("ErrorFailedToWriteInTempDirectory", $conf->diffusion->dir_temp);
				...
				// Line of free text
				$newfreetext = '';
				$paramfreetext = 'DIFFUSION_MYOBJECT_FREE_TEXT';
				...
				try {
					$odfHandler = new Odf(
						$srctemplatepath,
						array(
							'PATH_TO_TMP'	  => $conf->diffusion->dir_temp,
