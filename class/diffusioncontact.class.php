...
class DiffusionContact extends CommonObject
{
	...
	/**
	 * @var string 	If permission must be checkec with hasRight('diffusion', 'read') and not hasright('mymodyle', 'diffusioncontact', 'read'), you can uncomment this line
	 */
	public $element_for_permission = 'diffusion';

	/**
	 * @var string 	String with name of icon for diffusioncontact. Must be a 'fa-xxx' fontawesome code (or 'fa-xxx_fa_color_size') or 'diffusioncontact@diffusion' if picto is file 'img/object_diffusioncontact.png'.
	 */
	 ...
	 */
        ...
        {
		return $this->syncLink($this->fk_diffusion, $this->fk_contact, $this->contact_source, $this->fk_type_contact, $notrigger);
        }
	 ...
	 */
	public function syncLink($diffusionId, $contactId, $source, $typeContactId = 0, $notrigger = 0)
	{
		...
		if ($existing) {
			...
			$updateSql .= ', fk_type_contact = '.($typeContactId > 0 ? $typeContactId : 'NULL');
	...
	public function validate($user, $notrigger = 0)
	{
		...
		/* if (! ((!getDolGlobalInt('MAIN_USE_ADVANCED_PERMS') && $user->hasRight('diffusion', 'diffusioncontact', 'write'))
		 || (getDolGlobalInt('MAIN_USE_ADVANCED_PERMS') && $user->hasRight('diffusion', 'diffusioncontact_advance', 'validate')))
		 {
	...
	public function setDraft($user, $notrigger = 0)
	{
		...
		return $this->setStatusCommon($user, self::STATUS_DRAFT, $notrigger, 'DIFFUSION_MYOBJECT_UNVALIDATE');
	}
	...
	public function cancel($user, $notrigger = 0)
	{
		...
		return $this->setStatusCommon($user, self::STATUS_CANCELED, $notrigger, 'DIFFUSION_MYOBJECT_CANCEL');
	}
	...
	public function reopen($user, $notrigger = 0)
	{
		...
		return $this->setStatusCommon($user, self::STATUS_VALIDATED, $notrigger, 'DIFFUSION_MYOBJECT_REOPEN');
	}
	...
	public function getNomUrl($withpicto = 0, $option = '', $notooltip = 0, $morecss = '', $save_lastsearch_value = -1)
	{
		...
		if (empty($notooltip)) {
			...
			$linkclose .= ($label ? ' title="'.dolPrintHTMLForAttribute($label).'"' : ' title="'.dolPrintHTMLForAttribute($langs->trans("ShowDiffusionContact")).'"');
	...
	public function getNextNumRef()
	{
		...
		if (!getDolGlobalString('DIFFUSION_MYOBJECT_ADDON')) {
			$conf->global->DIFFUSION_MYOBJECT_ADDON = 'mod_diffusioncontact_standard';
		}

		if (getDolGlobalString('DIFFUSION_MYOBJECT_ADDON')) {
			$mybool = false;

			$file = getDolGlobalString('DIFFUSION_MYOBJECT_ADDON').".php";
			$classname = getDolGlobalString('DIFFUSION_MYOBJECT_ADDON');
