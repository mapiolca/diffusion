...
class Diffusion extends CommonObject
{
	...
	public $fields = array(
		...
		"entity" => array("type" => "integer", "label" => "Entity", "enabled" => "isModEnabled('multicompany')", 'position' => 10, 'notnull' => 1, "visible" => "0", "default" => "1", "index" => "1"),
		...
		"date_expedition" => array("type" => "datetime", "label" => "DateEnvoi", "enabled" => "1", 'position' => 505, 'notnull' => -1, "visible" => "1",),
		"tms" => array("type" => "timestamp", "label" => "DateModification", "enabled" => "1", 'position' => 501, 'notnull' => 0, "visible" => "-2",),
		"fk_user_creat" => array("type" => "integer:User:user/class/user.class.php", "label" => "UserAuthor", "picto" => "user", "enabled" => "1", 'position' => 510, 'notnull' => 1, "visible" => "-2", "csslist" => "tdoverflowmax150",),
		"fk_user_modif" => array("type" => "integer:User:user/class/user.class.php", "label" => "UserModif", "picto" => "user", "enabled" => "1", 'position' => 511, 'notnull' => -1, "visible" => "-2", "csslist" => "tdoverflowmax150",),
		"fk_user_exped" => array("type" => "integer:User:user/class/user.class.php", "label" => "UserExpedition", "picto" => "user", "enabled" => "1", 'position' => 512, 'notnull' => -1, "visible" => "-2", "csslist" => "tdoverflowmax150",),
	 ...
	 */
        public function __construct(DoliDB $db)
        {
			...
			if (empty($conf->diffusion->multidir_output[$entity])) {
				$conf->diffusion->multidir_output[$entity] = DOL_DATA_ROOT.($entity > 1 ? '/'.$entity : '').'/diffusion';
			}
			...
			$conf->diffusion->multidir_output[$entity] = DOL_DATA_ROOT.($entity > 1 ? '/'.$entity : '').'/diffusion';
               ...
               $diffusionoutput = $conf->diffusion->multidir_output[$entity];

               if (!empty($diffusionoutput)) {
                       dol_mkdir($diffusionoutput);
               }
		...
		/*if ($user->hasRight('diffusion', 'diffusion', 'read')) {
	...
	public function create(User $user, $notrigger = 0)
	{
		...
		if (!$result) {
			$this->error = $this->db->error()." sql=".$updatesql;
			$this->db->rollback();
		...
		if (!$notrigger) {
			...
			$result = $interface->run_triggers('DIFFUSION_CREATE', $this, $user, $langs, $conf);
			if ($result < 0) {
	...
	/**
	 * Sync generic object link with project in llx_element_element.
	 *
	 * @param	int	$projectid	Project id (0 to remove link)
	 * @return	int				Return 1 if OK, <0 if KO
	 */
	public function syncProjectObjectLink($projectid = 0)
	{
		$projectid = (int) $projectid;

		if ((int) $this->id <= 0) {
			$this->error = 'ErrorBadObjectIdForProjectLink';
			return -1;
		}

		$sql = "DELETE FROM ".MAIN_DB_PREFIX."element_element";
		$sql .= " WHERE (sourcetype = 'project' AND targettype = '".$this->db->escape($this->element)."' AND fk_target = ".((int) $this->id).")";
		$sql .= " OR (sourcetype = '".$this->db->escape($this->element)."' AND targettype = 'project' AND fk_source = ".((int) $this->id).")";
		...
		$sql .= " VALUES (".$projectid.", 'project', ".((int) $this->id).", '".$this->db->escape($this->element)."')";
	...
	{	
        ...
        if ($resql)
        {
            ...
            return $this->deleteCommon($user, $notrigger, 0, 'DIFFUSION_DELETE');
        }
        else
        {
            $this->error=$this->db->lasterror();
	...
	public function validate($user, $notrigger = 0)
	{
		...
		/* if (! ((!getDolGlobalInt('MAIN_USE_ADVANCED_PERMS') && $user->hasRight('diffusion', 'diffusion', 'write'))
		 || (getDolGlobalInt('MAIN_USE_ADVANCED_PERMS') && $user->hasRight('diffusion', 'diffusion_advance', 'validate')))
		 {
		...
		if (!$error) {
			...
			if (preg_match('/^[\(]?PROV/i', $this->ref)) {
				...
				$sql = 'UPDATE '.MAIN_DB_PREFIX."ecm_files set filename = CONCAT('".$this->db->escape($this->newref)."', SUBSTR(filename, ".(strlen($this->ref) + 1).")), filepath = 'diffusion/".$this->element."/".$this->db->escape($this->newref)."'";
				$sql .= " WHERE filename LIKE '".$this->db->escape($this->ref)."%' AND filepath = 'diffusion/".$this->element."/".$this->db->escape($this->ref)."' and entity = ".((int) (!empty($this->entity) ? $this->entity : 1));
				$resql = $this->db->query($sql);
				if (!$resql) {
					$error++;
					$this->error = $this->db->lasterror();
				}
				$sql = 'UPDATE '.MAIN_DB_PREFIX."ecm_files set filepath = 'diffusion/".$this->element."/".$this->db->escape($this->newref)."'";
				$sql .= " WHERE filepath = 'diffusion/".$this->element."/".$this->db->escape($this->ref)."' and entity = ".((int) (!empty($this->entity) ? $this->entity : 1));
				$resql = $this->db->query($sql);
				if (!$resql) {
					$error++;
					$this->error = $this->db->lasterror();
				...
				$dirsource = $conf->diffusion->multidir_output[!empty($this->entity) ? (int) $this->entity : 1].'/'.$this->element.'/'.$oldref;
				$dirdest = $conf->diffusion->multidir_output[!empty($this->entity) ? (int) $this->entity : 1].'/'.$this->element.'/'.$newref;
				if (!$error && file_exists($dirsource)) {
					...
					if (@rename($dirsource, $dirdest)) {
						...
						$listoffiles = dol_dir_list($conf->diffusion->multidir_output[!empty($this->entity) ? (int) $this->entity : 1].'/'.$this->element.'/'.$newref, 'files', 1, '^'.preg_quote($oldref, '/'));
	...
	public function setDraft($user, $notrigger = 0)
	{
		...
		/* if (! ((!getDolGlobalInt('MAIN_USE_ADVANCED_PERMS') && $user->hasRight('diffusion','write'))
		 || (getDolGlobalInt('MAIN_USE_ADVANCED_PERMS') && $user->hasRight('diffusion','diffusion_advance','validate'))))
		 {
		...
		$this->actionmsg = $this->actionmsg2;
		return $this->setStatusCommon($user, self::STATUS_DRAFT, $notrigger, 'DIFFUSION_BACKTODRAFT');
	}
	...
	public function setSent($user, $notrigger = 0)
	{
		...
		$result = $this->setStatusCommon($user, self::STATUS_SENT, $notrigger, 'DIFFUSION_SETDIFFUSED');
		if ($result > 0) {
			$sql = "UPDATE ".MAIN_DB_PREFIX."diffusion";
			$sql .= " SET date_expedition = '".$this->db->idate(dol_now())."', fk_user_exped = ".((int) $user->id);
			$sql .= " WHERE rowid = ".((int) $this->id);

			$resql = $this->db->query($sql);
			if ($resql) {
				$this->date_expedition = dol_now();
				$this->fk_user_exped = (int) $user->id;
			} else {
				$this->error = $this->db->lasterror();
				return -1;
			}
	...
	public function getTooltipContentArray($params)
	{
		...
		if (!empty($this->fk_user_exped)) {
			...
			if ($userexped->fetch($this->fk_user_exped) > 0) {
				$datas['sender'] = '<br><b>'.$langs->trans('UserExpedition').':</b> '.dol_escape_htmltag($userexped->getFullName($langs));
			}
		}
		if (!empty($this->date_expedition)) {
			$datas['date_expedition'] = '<br><b>'.$langs->trans('DateEnvoi').':</b> '.dol_print_date($this->date_expedition, 'dayhour', 'tzuser', $langs);
		}
	...
	public function getNomUrl($withpicto = 0, $option = '', $notooltip = 0, $morecss = '', $save_lastsearch_value = -1)
	{
		...
		$canreadobject = (!empty($this->id) && isModEnabled('diffusion') && is_object($user) && (!empty($user->admin) || $user->hasRight('diffusion', 'diffusion', 'read') || $user->hasRight('diffusion', 'read')));
	...
	public function getNextNumRef()
	{
		...
		if (!getDolGlobalString('DIFFUSION_DIFFUSION_ADDON')) {
			$conf->global->DIFFUSION_DIFFUSION_ADDON = 'mod_diffusion_standard';
		}

		if (getDolGlobalString('DIFFUSION_DIFFUSION_ADDON')) {
			$mybool = false;

			$file = getDolGlobalString('DIFFUSION_DIFFUSION_ADDON').".php";
			$classname = getDolGlobalString('DIFFUSION_DIFFUSION_ADDON');

			// Include file with class
			$dirmodels = array_merge(array('/'), (array) $conf->modules_parts['models']);
			foreach ($dirmodels as $reldir) {
				$dir = dol_buildpath($reldir."core/modules/diffusion/");
	...
	public function generateDocument($modele, $outputlangs, $hidedetails = 0, $hidedesc = 0, $hideref = 0, $moreparams = null)
	{
		...
		$modelpath = "core/modules/diffusion/doc/";
